...
app.layout = html.Div([
    ...
    dcc.RadioItems(
		options=[
			{'label': "hide the option", 'value': 'toHide'},
			{'label': "show the option to choose counties", 'value': 'toShow'},
		],
		value='toHide',
...
@app.callback(
        Output('GUI_lurky_div', 'style'), #is enclosing it with square braces so fatal? Yes....
        [Input('GUI_lurk_switch','value')]
    )
def hideCountrySelection(toHideCountryListStr):
	if toHideCountryListStr == "toHide":
		return {"display": "none"}
	else:
